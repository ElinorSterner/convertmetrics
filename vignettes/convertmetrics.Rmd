...
```
Suppose that we also want to convert `c(3, NA, 5) grams` to `taels`, so we will set the `num` argument equal to `c(3, NA, 5)`, `starting_unit` equals to `g`, and `final_unit` equals to `tael`. The function will disregard the NA values.
```{r}
